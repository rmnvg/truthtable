...
def test_run_query_against_real_duckdb_table():
    ...
    columns, rows, executed_sql = run_query(
        con, "SELECT id, name, salary FROM employees ORDER BY id"
    )
    ...
    assert executed_sql == "SELECT id, name, salary FROM employees ORDER BY id LIMIT 500"
